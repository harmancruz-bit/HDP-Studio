...
export default function LibrarySection({ product, isExpanded, isSelected, onSelectProduct, onSelectModule }: { product: Product; isExpanded: boolean; isSelected: boolean; onSelectProduct: () => void; onSelectModule: (module: string) => void }) {
  ...
  return <section className={`library-section ${isExpanded ? 'is-open' : ''} ${isSelected ? 'is-selected' : ''}`} style={{ '--product-accent': product.accent } as React.CSSProperties}>
    <button className="section-header" onClick={onSelectProduct} aria-expanded={isExpanded}>
    ...
    <div className="section-items"><div>{product.modules.map(module => <button key={module.name} className="library-item" onClick={() => onSelectModule(module.name)}><span className="item-icon">{module.icon}</span><span className="item-copy"><b>{module.name}</b><small>{module.description}</small></span><span className="module-count">{module.blocks}</span><Glyph name="chevron" className="module-chevron" /></button>)}</div></div>
  </section>
}
