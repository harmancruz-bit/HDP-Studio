...
export default function CanvasToolbar() {
  ...
  return <div className="canvas-toolbar" aria-label="Canvas controls">
    <div className="toolbar-cluster device-picker">{devices.map(item => <button key={item} className={device === item ? 'active' : ''} onClick={() => setDevice(item)}><span className={`device-symbol ${item.toLowerCase()}`} />{item}</button>)}</div>
